...
partial def shuffle (data: ShuffleData) (currentPosition: Nat) (limit: Nat) : ShuffleData :=
  (currentPosition < limit) ? data : 
    let currentAlphabet := data.alphabet
    let saltReminder := data.saltReminder % data.salt.length
    let asciiValue := (data.salt.get! ⟨saltReminder⟩).toNat
    ...
    let data' := ⟨currentAlphabet'', data.salt, cumulativeValue, (saltReminder + 1)⟩
    shuffle data' (currentPosition - 1) limit


def consistentShuffle (alphabet: String) : (String -> String)
  | "" => alphabet
  | salt => let initial := ⟨alphabet.toList, salt, 0, 0⟩
...
def Hashids.adjustAlphabetAndSeparators (hashids : Hashids) 
  ...
  (shuffledSeparators.isEmpty || (alphabetWithoutSeparators.length / shuffledSeparators.length).toFloat > separatorDivisor) 
    ? let sepsLength := calculateSeparatorsLength alphabetWithoutSeparators
      (sepsLength > shuffledSeparators.length) 
        ? let difference := sepsLength - shuffledSeparators.length
          ...
          ⟨consistentShuffle alpha hashids.salt, seps, ""⟩
        : let seps := shuffledSeparators.extract 0 ⟨sepsLength⟩
      ⟨consistentShuffle alphabetWithoutSeparators hashids.salt, seps, ""⟩
    : ⟨consistentShuffle alphabetWithoutSeparators hashids.salt, shuffledSeparators, ""⟩


def minimalAlphabetLength := 16

def Hashids.calculateAlphabetAndSeparators (hashids : Hashids) 
                                           (userAlphabet : String) : AlphabetAndSeparators := 
...
partial def Hashids.hashAux (number: UInt64) 
  ...
  (data.current <= 0) ? data : 
    let hashCharacter := alphabet[(data.current % alphabet.length).toNat]!
    let current := data.current / alphabet.length.toUInt64
    hashAux number alphabet ⟨s!"{hashCharacter}{data.hash}", current⟩

def Hashids.hash (input: UInt64) (alphabet: String): String := 
  (hashAux input (alphabet.toList) ⟨"", input⟩).hash
...
partial def Hashids.initialEncode (hashids : Hashids)
  ...
  (currentIndex >= numbers.length) ? (currentReturnString, alphabet) : 
    let currentNumber := numbers[currentIndex]!
    let buffer := bufferSeed ++ hashids.salt ++ alphabet
    let alphabet' := consistentShuffle alphabet (buffer.extract 0 ⟨alphabet.length⟩)
    let last := hash currentNumber alphabet'
    let returnString := (currentIndex + 1 >= numbers.length) ? currentReturnString ++ last :
      let nextNumber := currentNumber.toNat % ((last.get! 0).toNat + currentIndex)
...
def Hashids.addGuardsIfNecessary (hashids : Hashids)
  ...
  (encodedString.length >= hashLength) ? encodedString :
    let guard := hashids.guards.toList[hashids.guardIndex numbersHash encodedString 0]!
    let returnString := encodedString ++ guard.toString
    (returnString.length >= hashLength) ? returnString : 
      let guard' := hashids.guards.toList[hashids.guardIndex numbersHash returnString 2]!
...
def Hashids.encode (hashids : Hashids) : List UInt64 → Except String String
  | [] => Except.ok ""
  | xs => (xs.contains MAX_INTEGER)
    ? Except.error s!"number can not be greater than {MAX_INTEGER}" 
    : let len := hashids.alphabet.length
...
def Hashids.extractLotteryCharAndHashs (hashids : Hashids) 
  ...
  let i : Nat := (initialSplit.length = 2 || initialSplit.length = 3) ? 1 : 0
  let ithElementOfSplit : String := initialSplit.get! i
  let lotteryChar : Char := ithElementOfSplit.get! 0
  let breakdown : List String := ithElementOfSplit.extract ⟨1⟩ ⟨ithElementOfSplit.length⟩
    |>.replace separatorsRegex " " |>.splitOn " "
...
partial def Hashids.unhashAux (input: List Char) 
  ...
  (currentIndex >= input.length) ? currentNumber :
    let position := alphabet.find (· == input[currentIndex]!) |>.byteIdx
    ...
    match hashes with
      ...
      | h :: t =>
        ...
        let alphabet' := consistentShuffle alphabet 
          (buffer.extract 0 ⟨alphabet.length⟩)
...
def Hashids.decode (hashids : Hashids) (hash : String) : Except String (List UInt64) :=
  hash.isEmpty ? Except.ok [] :
    let guardsRegex := hashids.guards -- s!"[{guards}]"
    ...
    match hashids.encode returnValue with
      | Except.ok s => Except.ok ((s == hash) ? returnValue : [])
